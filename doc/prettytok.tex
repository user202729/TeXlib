...
\ProvidesFile{prettytok.tex} [2022/08/05 v0.1.0 ]
...
\begin{variable}{mode=}
	Specify the working mode of the package. It can either be:
\begin{itemize}
	\item |html|: output to a HTML file named for example |pretty-abc.html| (although this can be customized) if the main \TeX\ file is named |abc.tex|.
...
\end{itemize}
Pass like a normal option. For example |\usepackage[mode=term-shell]{prettytok}|.
\end{variable}

\subsubsection{HTML configuration}

These options are only available if |mode=html|.

\begin{variable}{html-file-name=}
	The output file name. Defaults to |pretty-|\meta{jobname}|.html|, as mentioned above.
\end{variable}

\begin{variable}{html-refresh-strategy=}
...
If terminal output is selected, the output is on the terminal, no special action required. (although you may have to enable either |--8bit| or |--shell-escape|)
...
\begin{itemize}
	...
	\item Printing to the console is rather limited (difficult syntax highlighting/scrolling/line wrapping), and most likely cluttered with the traceback/other \TeX\ default output.
