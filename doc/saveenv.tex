...
\begin{itemize}
	...
	\item \pkg{filecontentsdef} and \pkg{newverbs} provides very similar facilities (\env{filecontentsdefmacro} environment and \tn{Collectverbenv} command respectively); however it requires \verb+\endlinechar=13+ and does not support |\ExplSyntaxOn| environment.
...
will save the string \texttt{123\meta{newline}456} globally into |\data|.
...
some other content
\end{verbatim}
the effect would be identical to as if |\data| is set to \texttt{123\meta{newline}456},
...
Same as above, but the trailing whitespaces are preserved.
