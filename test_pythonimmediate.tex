...
\pythonimmediateverb|pythonimmediate.run_block_local(r'\pretty:n {3.xxx~ =~ ' + x + '}')|

\begin{pycode}
pythonimmediate.run_block_local(r'''
\pretty:n {4.}
\pythonimmediateverb|pythonimmediate.run_tokenized_line_local(r'\pretty:n{5.}')|
\pretty:n {6.}
''')
\end{pycode}
...
import unittest
from pythonimmediate import TokenList, Catcode, ControlSequenceToken, frozen_relax_token
...
""".count(' '), 3)
	...
	def test_expand(self):
		...
		self.assertEqual(TokenList.doc(' ').expand_o(), TokenList.doc(' '))
		self.assertEqual(TokenList.doc(' ').expand_x(), TokenList.doc(' '))
		self.assertEqual(1, len(TokenList.doc(' ')))
		self.assertEqual(TokenList([frozen_relax_token]).expand_x(), TokenList([frozen_relax_token]))
...
\pyfile{test_pythonimmediate_file.py}
