	...
	def test_tokens_control_chars(self):
		for s in [
			...
			for t in [
				...
				if s==chr(0) and isinstance(t, ControlSequenceToken):
					continue  # LuaLaTeX bug https://tex.stackexchange.com/questions/640267/lualatex-does-not-handle-control-sequence-consist-of-a-single-null-character-cor
				with self.subTest(s=s, t=t):
					pythonimmediate.debug("trying token", t)
					t.put_next()
					assert Token.get_next()==t
