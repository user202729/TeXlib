...
local function getvarsexpr_count(expr)  -- similar to above, but works on expression (allow two consecutive ##, and does not allow trailing #)
	local result={}
	local i, l=1, #expr
	while i<=l do
...
local function getvarsexpr(expr)  -- instead of return {tok → count}, return {tok → true}
	local result=getvarsexpr_count(expr)
...
local function next_st(linenumber)
	...
	return faketoken(imperative_prefix..preprocess_line_number(linenumber)..used_st[linenumber])
end
...
local function finalize_generate_code(statement, paramtext, replacementtext)  -- the macro itself is included in paramtext
	...
	local standard_paramtext_=standard_paramtext(statement)
	assert(#paramtext>=#standard_paramtext_)
	assert(tl_equal(
		standard_paramtext_,
		slice(paramtext, 1, #standard_paramtext_)
		))
	local paramtext_is_trivial=(#paramtext==#standard_paramtext_)


	paramtext=slice(paramtext, 1)
	replacementtext=slice(replacementtext, 1)

	-- do an optimization here
	-- that is, if the argument to be grabbed will be put next immediately, no point grabbing it
	-- probably must do here because .needsequence information is only kept up to this point...
	if paramtext_is_trivial then
		local use_count=getvarsexpr_count(replacementtext)  -- { replacementtext variable .tok → count }

		local i=#statement.needsequence

		while i>0 do
			local k, v=table.unpack(statement.needsequence[i])
			local n=v.get_nextvalue(tokenfromtok(k))

			-- n is what the corresponding part of the input stream would contain before this statement is called
			-- i.e. generated by the caller of this statement
			-- if this statement generates the same thing, then the common part can be eliminated
			if use_count[k]==1 and
				#replacementtext>=#n and tl_equal(n, slice(replacementtext, #replacementtext-#n+1))
			then
				replacementtext=slice(replacementtext, 1, #replacementtext-#n)
				paramtext=slice(paramtext, 1, #paramtext-#v.paramtext(tokenfromtok(k)))
				i=i-1
			else
				break
			end
		...
		--while true do
		--	local i=#paramtext
		--	while not is_paramsign(paramtext[i]) do i=i-1 end
		--	assert(i~=#paramtext)

		--	if tl_equal(
		--		slice(paramtext, i),
		--		slice(replacementtext, #replacementtext-#paramtext+i)
		--	) then
		--		replacementtext=slice(replacementtext, 1, #replacementtext-#paramtext+i)
		--		paramtext=slice(paramtext, 1, i),
		--	else
		--		break
		--	end
		--end
		...
		--while #paramtext>0 and #replacementtext>0 and
		--	paramtext[#paramtext].tok==replacementtext[#replacementtext].tok
		--do
		--	paramtext[#paramtext]=nil
		--	replacementtext[#replacementtext]=nil
		--end
	end
	...
	-- excessive logging
	--replacementtext=cati({faketoken "prettye:n", bgroup, paramtext[1], egroup}, replacementtext)
...
standard_paramtext=function(statement)
...
local function standard_nextvalue(next_statement)
	...
	for k, v in pairs(next_statement.need) do
		nextvalue[k]=next_statement.computable[k].get_nextvalue(tokenfromtok(k))
	end
	return nextvalue
end
...
local command_handler={
	...
	matchrm=function(selftoken, lastlinenumber, stack, add_statement, context)
		...
		if #group==1 and is_paramsign(group[1]) then
			errorx "matchrm{#} makes no sense"
		end
		add_statement {
			...
			generate_code=function(statement, statements)
				...
				for _, kv in ipairs(statements[n].needsequence) do
					if kv[2].get_expandafter_sequence==nil then
						errorx("expandonce but some parameters (e.g. ",
							tokenfromtok(kv[1])
							,") with type ",kv[2].debugname," are not expandafter-able")
					end
	...
	assigno=function(selftoken, lastlinenumber, stack, add_statement, context)  -- assigno #x {123}
		local varname=getvarname(stack)
		check_var_not_outer(varname, context)
		local content=getbracegroup(stack)
		add_statement {
	...
	assignoo=function(selftoken, lastlinenumber, stack, add_statement, context)
		local varname=getvarname(stack)
		check_var_not_outer(varname, context)
		local content=getbracegroup(stack)
		add_statement {
	...
	expcallpeek=function(selftoken, lastlinenumber, stack, add_statement, context)
		local content=getbracegroupinside(stack)
		if find_subsequence({faketoken "continueblock"}, content)==-1 then --cannot find
			errorx("no continueblock found in argument ", content)
		end
		-- we temporarily ignore the case that there are multiple continueblock
		add_statement {
			debug=cati({faketoken "expcallpeek"}, content),
			innerexpr=content,
			generate_code=function(statement, statements, innerexpr_compiled)
				local paramtext=standard_paramtext(statement)
				local replacementtext=standard_replacementtext(statements[statement.nextindex], standard_nextvalue(statements[statement.nextindex]))
				local offset=find_subsequence_checked({faketoken "continueblock"}, innerexpr_compiled)  -- this might actually fail despite the checking above, because continueblock inside another rfunction might be hidden after compilation
				finalize_generate_code(statement, paramtext,
					cati(
						slice(innerexpr_compiled, 1, offset),
						replacementtext,
						slice(innerexpr_compiled, offset+2)
					))
			end,
		}
	end,
	...
	prettyo=function(selftoken, lastlinenumber, stack, add_statement, context)
		local content=getbracegroupinside(stack)
		pushtostackfront(stack, {faketoken "expcall",
			bgroup, expandafter, faketoken "prettye:n", expandafter, bgroup, expandafter, faketoken("[L"..lastlinenumber.."]:")}, content, {egroup, egroup})
	end,
...
function generic_compile_code(functionname, body, passcontrol, statement_extra, outervar)
	...
	local labelfunctionpasscontrol=randomlabel()
	body=cati(body,
		{
			faketoken "label", labelfunctionend,
			faketoken "putnext", bgroup}, passcontrol, {egroup,
			faketoken "label", labelfunctionpasscontrol,
		})
	...
	local context={
		...
		labelfunctionpasscontrol=labelfunctionpasscontrol,
	...
	for i=1, #statements do
		...
		if i~=#statements then
			...
			table.insert(v.nextindices, v.nextindex)
		end
		...
		if v.conditionalgoto then
			...
			table.insert(v.nextindices, v.conditionalgoto)
		end
	end
	...
	.comefrom = list of statement indices that goes to this statement
	.nextindices = list of statement indices that this statement might go to
	...
	.produce: ↑ set of var tok produced **new** by that statement i.e. does not necessarily need from before
	.computable: ↑ set of var tok computable (have a value) *right before* that statement. Anything in need is in computable
	.reachable: {var.tok → {argtype → true}} if argtype is reachable *right before* that statement
	...
	-- initialize reachable value for entry point
	-- & propagate the `reachable` values

	local function propagate(j, tok, argt)
		local v=statements[j]
		assert(v.reachable[tok][argt])
		assert(argt~=argtype.deleted)  -- this should not happen

		if v.produce[tok] then argt=v.produce[tok] end
		if argt==argtype.deleted then return end  -- don't need to propagate this one

		for _, k in ipairs(v.nextindices) do
			local w=statements[k]
			if not w.reachable[tok] then w.reachable[tok]={} end
			if not w.reachable[tok][argt] then
				w.reachable[tok][argt]=true
				propagate(k, tok, argt)
			end
		end
	end
	...
	for tok, _ in pairs(outerscope) do
		if tok~="layer" and tok~="prefix" then
			statements[1].reachable[tok]={[argtype.normal]=true}
			propagate(1, tok, argtype.normal)
		end
	end

	for i, v in ipairs(statements) do
		for tok, argt in pairs(v.produce) do
			if argt~=argtype.deleted then
				if v.reachable[tok]==nil then v.reachable[tok]={} end
				if not v.reachable[tok][argt] then
					v.reachable[tok][argt]=true
					propagate(i, tok, argt)
				end
			end
		end
	end

	-- then compute `computable` value
	for i, v in ipairs(statements) do
		local computable={}
	...
	for i, v in ipairs(statements) do
		if v.innerexpr then
			...
			-- except for matchrm, computable and need are roughly the same
			for k, v in pairs(v.computable) do
				current_scope[k]={paramsign, tokenfromtok(k)}
			end
			
			v.innerexpr_compiled, v.need=compile_outer(v.innerexpr, current_scope)
			v.need=getvarsexpr(v.innerexpr_compiled)
	...
	for _, v in ipairs(statements) do
		local n={}
		for var_tok, _ in pairs(v.need) do
			-- var_tok: .tok value
			-- var_kind: for example argtype.Ntype or argtype.normal
			local var_kind=v.computable[var_tok]
			assert(var_kind~=nil)
			n[#n+1]={var_tok, var_kind}
		end
		table.sort(n, function(a, b)
			if type(a[1])~=type(b[1]) then
				return type(a[1])<type(b[1])
			end
			return a[1]<b[1]
		end)  -- TODO maybe make something better?
		v.needsequence=n
	end
	...
	for _, v in ipairs(statements) do
		...
		if assigno_tok~=nil then
			...
			if not okay then
				prettyprint("Warning: variable ", tokenfromtok(assigno_tok), " assigned but not used")
				assert(not statements[v.nextindex].need[assigno_tok])
				statements[v.nextindex].need[assigno_tok]=argtype.normal
				local len=#n
		...
		for i, v in ipairs(statements) do
			...
			if v.need then
				...
				--for k, v in pairs(v.need) do
					table.insert(s, tokenfromtok(k))
					if v.is_a[argtype.Ntype] then table.insert(s, "^") end  -- just for debug purpose
					table.insert(s, ",")
				end
			end
			table.insert(s, "/")

			table.insert(s, "produce=")
			if v.produce then
				for k, w in pairs(v.produce) do
					table.insert(s, tokenfromtok(k))
					table.insert(s, ":"..w.debugname..",")
				end
			end
			table.insert(s, "/")

			table.insert(s, "computable=")
			if v.produce then
				for k, w in pairs(v.computable) do
					table.insert(s, tokenfromtok(k))
					table.insert(s, ":"..w.debugname..",")
				end
			end
	...
	finalize_generate_code(statements[#statements], {statements[#statements].caller}, {})
...
local function compile_block(stack, body, scope, lastlinenumber,
	passcontrol,  -- same meaning as in generic_compile_code, e.g. is \exp_end: for r-function
	allow_take_var   -- bool: true/false (true: block, false: function)
	)
	local localfn=next_st(lastlinenumber)

	local initial_param=getuntilbrace(stack)
	local content=getbracegroupinside(stack)

	if #initial_param>0 then
		local matchrm=faketoken "matchrm"
		matchrm._linenumber=lastlinenumber
		content=cati({matchrm}, wrapinbracegroup(initial_param), content)
	end

	local needsequence=generic_compile_code(localfn, content, passcontrol, {}, scope)
	-- TODO somehow mark this function as internal

	body[#body+1]=localfn
	for _, v in pairs(needsequence) do
		local tok=v[1]
		if not allow_take_var then
			errorx("error: function should not capture outer variable ", tokenfromtok(tok))
		end
...
local compile_outer_handler={
	imperativesetprefix=function(stack, body, scope, lastlinenumber)
		...
		imperative_prefix=prefix  -- global variable
	end,

	rblock=function(stack, body, scope, lastlinenumber)
		return compile_block(stack, body, scope, lastlinenumber, {exp_end}, true)
	end,
	zblock=function(stack, body, scope, lastlinenumber)
		return compile_block(stack, body, scope, lastlinenumber, {}, true)
	end,
	rfunction=function(stack, body, scope, lastlinenumber)
		return compile_block(stack, body, scope, lastlinenumber, {exp_end}, false)
	end,
	zfunction=function(stack, body, scope, lastlinenumber)
		return compile_block(stack, body, scope, lastlinenumber, {}, false)
	end,
...
-- * the need table i.e. {t.tok=true, ...}   which is subset of scope
...
function metadef_call()
	local header=token.scan_toks()
	if #header==0 then error("header cannot be empty") end
	local functionname=header[1]
	assert(functionname.csname and not functionname.active, "metadef must use control sequence")
	functionname=functionname.csname
	assert(command_handler[functionname]==nil, "command "..functionname.." already defined")

	local body=token.scan_toks()

	local args_=slice(header, 2)
	assert(#args_%2==0, "wrong arg specification")

	local args={}
	local arg_seen={}
	for i=1, #args_, 2 do
		assert(is_paramsign(args_[i]), "wrong arg specification, token "..i.." should be #")
		local arg=args_[i+1]
		assert(not is_paramsign(arg), "wrong arg specification, token "..i.." should not be #")
		if arg_seen[arg.tok] then
			errorx("duplicate arg ", arg)
		end
		arg_seen[arg.tok]=true
		args[#args+1]=arg
	end

	command_handler[functionname]=function(selftoken, lastlinenumber, stack, add_statement, context)
...
function optimize_pending_definitions()
	...
	local function find_def_generic(t, caller)  -- given a token, return (index, t[index]), or (nil, nil)
	...
	-- replace all occurrences of ⟨search⟩ token by ⟨replace⟩. As if ⟨search⟩ is \let to ⟨replace⟩
	...
	while true do
		...
		for _, v in ipairs(pending_definitions) do if v.z_optimizable then
			...
			if #paramtext==1 and #replacementtext==1 then
				...
				if v.is_internal then
					-- optimize by replacing macro X with macro Y if X is defined to be Y without any arguments
					replace_macro(caller, replacement)
					done_anything=true
				else
					-- in this case... we make X's definition identical to Y and hope Y can be eliminated later
					local _, t=find_def_generic(pending_definitions, replacement)
					if t~=nil then
						v.paramtext2=t.paramtext2
						assert(v.paramtext2~=nil)
						v.replacementtext=t.replacementtext
						v.paramtext=cati(v.paramtext, v.paramtext2)
					end
				end
			end
		end end
		...
		--for i=1, #pending_definitions do local v=pending_definitions[i] if v and v.z_optimizable then
			...
			if target_replacementtext_pos~=1 and target and target.csname==nil and not is_paramsign(target) and not is_r_expansion then
				...
				errorx("weird? expandafter chain do nothing (target token is ", target,") (in function ", v.caller, ")")
			end
